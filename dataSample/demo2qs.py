...
# 配置日志
logging.basicConfig(
    filename='university_rankings_qs_import.log',
    level=logging.INFO,
...
def connect_to_mysql(host='localhost', user='root', password='junw', database='computer_rank', charset='utf8mb4'):
    ...
    try:
        ...
        logging.error(f"无法连接到 MySQL 数据库: {e}")
        raise

def process_txt_file(file_path, connection):
    """
    处理单个 TXT 文件并将数据插入数据库。
    """
        ...
        for line_num, line in enumerate(data_lines, start=2):
            ...
            try:
                ...
                if len(tags) == 2:
                    ...
                    university_tags = tags[0].strip()
                    university_tags_state = None  # 如果没有洲信息
                ...
                # 在 QS 排名中，rank_variant 可以固定为 'QS'，以区分不同的排名来源
                rank_variant = 'QS'
                ...
                logging.error(f"文件 {file_path} 第 {line_num} 行插入数据库时出错 (IntegrityError): {ie} | 数据: {line}")
            except ProgrammingError as pe:
                logging.error(f"文件 {file_path} 第 {line_num} 行插入数据库时出错 (ProgrammingError): {pe} | 数据: {line}")
            except Exception as e:
...
def main():
    ...
    if not txt_files:
        logging.warning("当前目录下没有找到任何 txt 文件。")
    ...
    logging.info("程序运行结束。")
    print("数据导入完成。请查看日志文件 'university_rankings_qs_import.log' 获取详细信息。")
